		...
		<view class="orders">
			<view class="box">
				<view class="label" v-for="(row,index) in orderTypeLise" :key="row.name" hover-class="hover" hover-stay-time="50" @tap="toOrderType(index)">
					<view class="icon"><view class="badge" v-if="row.badge>=0">{{cartCount}}</view><image :src="'../../static/HM-PersonalCenter/'+row.icon"></image></view>
					{{row.name}}
					<!-- {{row.badge}} -->
				</view>
			</view>
	...
	export default {
		...
		methods: {
			init() {
				...
				// this.orderTypeLise[0].badge = this.cartCount;
			},
			//用户点击订单类型
			toOrderType(index){
				if (this.orderTypeLise[index].name == "待付款") {
					uni.navigateTo({
